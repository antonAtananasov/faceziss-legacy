...
def processVideo(images:npt.NDArray, fps:int, magnificationParams:dict = {}):
    magnificationParams['fps'] = fps

    magnificationParams['kernel'] = gaussian_kernel if not "kernel" in magnificationParams.keys() else magnificationParams["kernel"]
    magnificationParams['level'] = 4 if not "level" in magnificationParams.keys() else magnificationParams["level"]
    magnificationParams['alpha'] = 100 if not "alpha" in magnificationParams.keys() else magnificationParams["alpha"]
    magnificationParams['freq_range'] = [0.833, 2] if not "freq_range" in magnificationParams.keys() else magnificationParams["freq_range"] # [high omega, low omega] 
    print(magnificationParams['freq_range'])
    magnificationParams['attenuation'] = 1 if not "attenuation" in magnificationParams.keys() else magnificationParams["attenuation"]
    mode = ['gaussian', 'laplacian'][0]

    magnificationParams['images'] = images if not "images" in magnificationParams.keys() else magnificationParams["images"]

    if mode == 'gaussian':
        output_video = gaussian_evm(**magnificationParams)
        return output_video
    else:
        magnificationParams['lambda_cutoff'] = 1000 if not "lambda_cutoff" in magnificationParams.keys() else magnificationParams["lambda_cutoff"]
